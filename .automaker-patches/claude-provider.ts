...
export class ClaudeProvider extends BaseProvider {
  ...
  async *executeQuery(options: ExecuteOptions): AsyncGenerator<ProviderMessage> {
    ...
    // Privacy Guard: Generiere Session-ID für Anonymisierung
    const privacySessionId =
      sdkSessionId || `session-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;

    // Build prompt payload with Privacy Guard anonymization
    let promptPayload: string | AsyncIterable<any>;

    if (Array.isArray(prompt)) {
      // Multi-part prompt (with images) - anonymize text parts
      const anonymizedPrompt = await Promise.all(
        prompt.map(async (part) => {
          if (typeof part === 'object' && part.type === 'text' && part.text) {
            return { ...part, text: await PrivacyGuard.anonymize(privacySessionId, part.text) };
          }
          return part;
        })
      );
      ...
      // Simple text prompt - anonymize
      promptPayload = await PrivacyGuard.anonymize(privacySessionId, prompt);
    }
    ...
    try {
      ...
      // Stream messages with Privacy Guard deanonymization
      for await (const msg of stream) {
        // Deanonymize text content in messages
        const deanonymizedMsg = this.deanonymizeMessage(msg as ProviderMessage, privacySessionId);
        yield deanonymizedMsg;
      }
    } catch (error) {
  ...
  /**
   * Privacy Guard: Deanonymize message content
   */
  private deanonymizeMessage(msg: ProviderMessage, sessionId: string): ProviderMessage {
    if (!msg) return msg;

    // Deep clone to avoid mutating original
    const result = JSON.parse(JSON.stringify(msg));

    // Deanonymize based on message type
    if (result.message?.content) {
      if (typeof result.message.content === 'string') {
        result.message.content = PrivacyGuard.deanonymize(sessionId, result.message.content);
      } else if (Array.isArray(result.message.content)) {
        result.message.content = result.message.content.map((part: any) => {
          if (part.type === 'text' && part.text) {
            return { ...part, text: PrivacyGuard.deanonymize(sessionId, part.text) };
          }
          return part;
        });
      }
    ...
    // Deanonymize tool results
    if (result.tool_result?.content) {
      result.tool_result.content = PrivacyGuard.deanonymize(sessionId, result.tool_result.content);
    }
